module.exports = {
      ...
      try {
        let result = await db
          ...
          .findOneAndUpdate(
            findObject,
            { $inc: updateObject },
            { upsert: upsert, returnNewDocument: updatedDoc }
          );
        resolve(result);
      } catch (error) {
        reject(error);
  ...
  updateOne: async (db, collectionName, filter, updateObject, upsert) => {
    if (
      !updateObject ||
      ...
      throw new Error("Update object is invalid or null.");
    }

    try {
      let result = await db
        .collection(collectionName)
        .updateOne(filter, { $set: updateObject }, { upsert: upsert });
      return result;
    } catch (error) {
  ...
  updateMany: async (db, collectionName, findObject, updateObject, upsert) => {
    return new Promise(async (resolve, reject) => {
      try {
        let result = await db
          .collection(collectionName)
          .updateMany(findObject, { $set: updateObject }, { upsert: upsert });
        resolve(result);
      } catch (error) {
        reject(error);
      }
